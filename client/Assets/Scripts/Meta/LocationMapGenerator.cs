...
namespace Pets.Meta
{
    ...
    /// The graph produced here is walkable (LocationMapTraversal) and rendered by
    /// LocationMapController, but what a node actually *does* on arrival isn't modeled yet — node
    /// resolution (fights, events, camps) is the next Phase 1 step (PLAN.md).</summary>
    public static class LocationMapGenerator
    {
        ...
        public static LocationMap Generate(int seed, int layerCount = DefaultLayerCount)
        {
            if (layerCount < 3)
            {
            ...
            var map = new LocationMap { LayerCount = layerCount, Seed = seed };
            ...
            var start = new LocationMapNode { Id = "L0-0", Type = NodeType.PvE, Layer = 0, IndexInLayer = 0 };
            map.Nodes.Add(start);
            map.StartNodeId = start.Id;

            var previousLayer = new List<LocationMapNode> { start };
            for (int layer = 1; layer < layerCount - 1; layer++)
            {
                ...
                var currentLayer = new List<LocationMapNode>();
                var typeCountsInLayer = new Dictionary<NodeType, int>();
                for (int i = 0; i < nodeCount; i++)
                {
                    var type = PickNodeType(rng, typeCountsInLayer, campAllowed);
                    currentLayer.Add(new LocationMapNode { Id = $"L{layer}-{i}", Type = type, Layer = layer, IndexInLayer = i });
                }
            ...
            var gym = new LocationMapNode { Id = $"L{layerCount - 1}-0", Type = NodeType.Gym, Layer = layerCount - 1, IndexInLayer = 0 };
            map.Nodes.Add(gym);
            map.GymNodeId = gym.Id;
            ConnectLayers(previousLayer, new List<LocationMapNode> { gym }, rng);
        ...
        private static void ConnectLayers(List<LocationMapNode> from, List<LocationMapNode> to, DeterministicRandom rng)
        {
        ...
        private static (int lo, int hi) WidenForTypeDiversity(List<LocationMapNode> to, int lo, int hi, int minLo, int maxHi)
        {
        ...
        private static void ApplyRareSingleConnection(DeterministicRandom rng, List<LocationMapNode> to, (int lo, int hi)[] ranges)
        {
            if (ranges.Length <= 1)
            {
                return;
