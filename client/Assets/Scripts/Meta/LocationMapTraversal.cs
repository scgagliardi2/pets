...
namespace Pets.Meta
{
    /// <summary>The player's walk across a generated LocationMap (design doc §5): where they stand,
    /// which nodes that opens up, and the path taken to get there. Kept separate from LocationMap so
    ...
    /// to be a field on LocationMapController, which meant leaving the map scene for the Team screen
    ...
    public sealed class LocationMapTraversal
    {
        ...
        public LocationMapTraversal(LocationMap map, List<string> visitedNodeIds)
        {
        ...
        public static LocationMapTraversal ForRun(RunState run, int seed, int layerCount)
        {
            if (run.LocationMap == null)
            {
                run.LocationMap = LocationMapGenerator.Generate(seed, layerCount);
                run.VisitedMapNodeIds.Clear();
            }
            return new LocationMapTraversal(run.LocationMap, run.VisitedMapNodeIds);
        }

        /// <summary>Replaces the run's map with a freshly generated one and restarts the walk.
        /// The Location Map's "New Map" button, which is a prototyping affordance rather than
        /// something a real run should offer (PLAN.md §6).</summary>
        public static LocationMapTraversal RegenerateForRun(RunState run, int seed, int layerCount)
        {
        ...
        public LocationMapNode CurrentNode => Map.GetById(CurrentNodeId);
        ...
        public IReadOnlyList<LocationMapNode> AvailableNextNodes =>
