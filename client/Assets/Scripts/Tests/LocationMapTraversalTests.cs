...
namespace Pets.Tests
{
    ...
    public class LocationMapTraversalTests
    {
        [Test]
        public void NewTraversal_StartsOnTheStartNode_WithThreeOptions()
        {
            var map = LocationMapGenerator.Generate(seed: 7);
            var traversal = new LocationMapTraversal(map);

            Assert.AreEqual(map.StartNodeId, traversal.CurrentNodeId);
            Assert.AreEqual(LocationMapGenerator.StartingOptionCount, traversal.AvailableNextNodes.Count);
        ...
        {
            var map = LocationMapGenerator.Generate(seed: 8);
            var traversal = new LocationMapTraversal(map);
        ...
        {
            var map = LocationMapGenerator.Generate(seed: 12);
            var traversal = new LocationMapTraversal(map);
        ...
        {
            var map = LocationMapGenerator.Generate(seed: 13);
            var traversal = new LocationMapTraversal(map);
        ...
        {
            var map = LocationMapGenerator.Generate(seed: 21);
            var traversal = new LocationMapTraversal(map);
            while (!traversal.IsComplete)
            {
        ...
        /// LocationMapController, so opening the Team screen and coming back re-ran Start, generated
        ...
        public void ForRun_ResumesTheSameMapAndPosition_WhenTheScreenIsReEntered()
        {
            ...
            var first = LocationMapTraversal.ForRun(run, seed: 31, layerCount: LocationMapGenerator.DefaultLayerCount);
            ...
            var resumed = LocationMapTraversal.ForRun(run, seed: 999, layerCount: LocationMapGenerator.DefaultLayerCount);
        ...
        {
            var run = new RunState();
            var traversal = LocationMapTraversal.ForRun(run, seed: 7, layerCount: LocationMapGenerator.DefaultLayerCount);
        ...
        {
            var run = new RunState();
            var first = LocationMapTraversal.ForRun(run, seed: 5, layerCount: LocationMapGenerator.DefaultLayerCount);
            first.MoveTo(first.AvailableNextNodes.First().Id);

            var fresh = LocationMapTraversal.RegenerateForRun(run, seed: 6, layerCount: LocationMapGenerator.DefaultLayerCount);
        ...
        {
            var map = LocationMapGenerator.Generate(seed: 22);
            var traversal = new LocationMapTraversal(map);
            while (!traversal.IsComplete)
            {
                traversal.MoveTo(traversal.AvailableNextNodes.First().Id);
