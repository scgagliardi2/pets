...
/pets
  ...
  /tools                      # One-off content/asset scripts (generate_ui_sprites.py,
                              #   fetch_roster_ids.py and fetch_evolution_chains.py — the roster's
                              #   cached PokeAPI id and evolution data)
  /docs
    ...
    roster_evolution_chains.json                  # GENERATED by tools/fetch_evolution_chains.py — chain depth + next form
    battle-sim-spec.md
    content-schema.md
...
**Status (as of 2026-09-13) — read this before trusting the phase list below.** The shell was built
...
xlsx→asset pipeline, with a Pokédex screen to browse them and a tier cap keeping Character
...
shield bubble and the opening moving to screen open (ADR 0016): **299 EditMode and 124 PlayMode, all
passing** — that last failure was the same beat-ordering problem and is fixed.
...
- **Team type synergies** (ADR 0015): `Simulation/TeamSynergy` applies one small rule per type in a
  ...
  lifesteal, status ward, status, charge owed). **The opening is applied as the battle screen opens**,
  not inside the first Step (`OnDemandStepRunner.ApplyOpening`, ADR 0016): the defences are 1-3 points
  and the first exchange spends them, so drawn only after `NextStep` returned they were invisible in
  every fight. `docs/type-passives.md` is the table of every synergy's numbers and how each scales,
  alongside the species passives.
...
- `Scripts/Meta` (pure C#): `RunState` (line-up/Box/Money/Morale/seed, `MoveMon`, `ReleaseMon`, the
  current and completed Locations, `TravelTo`, `EarnBadge`), the run-scale difficulty curve
...
- Screens, all generated by `Assets/Editor/*SceneBuilder.cs` and driven end-to-end by PlayMode
  ...
  - `RegionHub.unity` — shown after Character Select and after every Gym (ADR 0007): three Location
    ...
    eight Gyms is next and what tier and EXP of opposition to expect, and Morale/Money/Badges plus a Menu button in the
    title bar. Owns a `RunBootstrapper`, since Character Select hands off here. Travel is instant —
    the Trailblazer minigame isn't built. The Ingame Menu's "Back to Map" lands here while the run is
    between Locations (`SceneNavigator.MapOrHubFor`).
...
2. **One map model.** `RunState.Nodes`/`CurrentNodeIndex`/`AdvanceToNextNode`, `LocationNodeState`
   ...
   `VisitedMapNodeIds` (walked via `LocationMapTraversal.ForRun`) is the only one left. The walk is
...
final Location) and the EXP they carry (4 × badges) — with Legendaries only on the final Gym. 92 of
the 183 species have a real PokeAPI evolution link; the three branching lines (Eevee, Tyrogue,
...
- **Integration/PlayMode tests:** these drive the *saved scenes* — finding real objects by
  ...
  `CharacterSelectScenePlayModeTests`, `RegionHubScenePlayModeTests`, `LocationMapScenePlayModeTests` (walking *and* resolving —
...
- **The content-import pipeline exists** (ADR 0004), and it's how the roster got from 28
  ...
  - `tools/fetch_roster_ids.py` — one PokeAPI request resolving each sheet name to its National Dex
    ...
    species.
  - `tools/fetch_evolution_chains.py` — the real PokeAPI evolution chains for the same roster,
    cached in `docs/roster_evolution_chains.json`: each species' chain depth and what it evolves
    into, restricted to the roster. Branching lines (Eevee, Tyrogue, Nincada) are recorded as
    unresolved rather than given an arbitrary answer, since `EvolvesInto` is a single reference.
...
- **Evolution chains are wired** (ADR 0005): 92 species evolve into another roster species, and
  each carries its real chain depth in `EvolutionStage`, which is what finally gives a passive's
  `MagnitudeByStage` table the index it was always for.
...
- **The difficulty curve was derived by arithmetic, not by play or by a model.** ADR 0007's
  constants came from simulating whole runs; ADR 0008 replaced them with numbers you can count (a
...
9. ~~**Expand curated content past 28 species.**~~ Done — all 183 are imported (ADR 0004). What the
   ...
   - ~~**Filter the encounter pools.**~~ Done (ADR 0007, retargeted in ADR 0008): wild and Gym pools
     are base forms under a badge-scaled *tier* cap, with Legendaries only on the final Gym. The dev
     random battle still rolls from the whole roster, at the party's top EXP.
   ...
   - ~~**Wire the evolution chains**~~ — done (ADR 0005). What's left there is the **branching
     ...
     express a choice the player should be making.
