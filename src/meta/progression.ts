...
/** Beating another trainer pays better than a wild fight: it is the only reward on offer. */
export const MONEY_PER_TRAINER_WIN = 5;
...
export function moneyForWin(nodeType: 'Wild' | 'MysteryTrainer' | 'Gym'): number {
  if (nodeType === 'Gym') return MONEY_PER_GYM_WIN;
  if (nodeType === 'MysteryTrainer') return MONEY_PER_TRAINER_WIN;
