...
| `frontSprite` | `Sprite` reference (nullable) | The game-style battle sprite, front-facing, from `client/Assets/Art/Pokemon-Sprites/animated_sprites/front/{slug}.gif`. **This is what every screen draws**: cards, Pokédex, party strip, catch offers, the evolution overlay, and the foe's side of the battlefield. Assigned by name slug (not id) by the roster importer. Nullable because the set covers 181 of 183 species; `PokemonSprites.LoadFront` falls back to `sprite`. |
