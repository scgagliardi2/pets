...
| `sprite` | `Sprite` reference | The PokeAPI official artwork under `client/Assets/Art/Pokemon/{id}.png`. A direct reference, **not** a path string: only the sprites curated species point at are pulled into a build, and a wrong reference is visibly missing in the Inspector rather than a silent runtime null. Assigned by Id by the content-import pipeline (PLAN.md §8). No longer drawn by default — see `frontSprite` — but kept as the fallback beneath it. |
| `frontSprite` | `Sprite` reference (nullable) | The game-style battle sprite, front-facing, from `client/Assets/Art/Pokemon-Sprites/animated_sprites/front/{slug}.gif`. **This is what every screen draws**: cards, Pokédex, party strip, catch offers, the evolution overlay, and the foe's side of the battlefield. Assigned by name slug (not id) by the roster importer. Nullable so that a species added before its sprite still renders — `PokemonSprites.LoadFront` falls back to `sprite` — but every curated species has one today, and `ContentIntegrityTests` enforces that. |
| `backSprite` | `Sprite` reference (nullable) | The same sprite seen from behind, drawn **only** for the player's own Lead and Support on the battlefield — the over-the-shoulder view the main-series games use. Nullable on the same terms; `PokemonSprites.LoadBack` falls back to `frontSprite` first (same art style and scale, merely facing the wrong way) and only then to `sprite`. |
...
**As built** (ADR 0007), a deviation from the above: there is no `LocationTypeDefinition` asset yet.
