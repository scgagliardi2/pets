...
- **Morale** = the run's life total. Decrements on a lost battle (Gym or otherwise — TBD exactly which). Hitting 0 ends the run. **As built** (ADR 0007), each badge refills it to its starting value, so it budgets losses per Location.
- Beating a Location's Gym awards a badge and returns you to the **Region Hub** to pick the next Location.
- Run-ending "finale" condition: **as built** (ADR 0007), eight badges wins the run. Whether an Elite-Four/Champion-style capstone should follow the eighth Gym is still open — see Open Questions.
...
**Events as built** (ADR 0014): an Event node rolls one of four encounters, shown as a modal over the map with two or three choices, then what the choice did:
- **Legendary Sighting**: challenge a lone Legendary (a real fight, no catching; a win pays the usual win plus $15 and a random held item, a loss costs Morale), or slip away.
- **Game Corner**: $5 on the slots for a 50% shot at $20, or +$4 in loose coins.
- **Traveling Trader**: swap your least-grown mon for a random base form one tier up at the same EXP, or decline.
- **Team Rocket Ambush**: lose half your money, lose a random bag item, or grab their loot (50% a random item, otherwise −1 Morale).

This is the one place a Legendary appears before the final Gym.
...
- **What the sheet's numbers mean in a fight** (as built, ADR 0008 + ADR 0009): **nothing directly**. The sheet keeps the real Pokémon stats, and the importer turns each species' base-stat *total* into a **tier** (1–6), then splits that tier's point budget — 8 points at tier 1, +10 a tier — across Attack, Health and Speed in the ratio of the species' own real stats, capped so Health always beats Attack (`Data/SpeciesTier`). Charmander's 52/39/65 becomes **3/4/1**; Magikarp's 10/20/80 becomes **2/5/1**. Speed is 1 for most of the roster and never above 3. The sheet's real Attack:Health ratio also sets each species' **growth value** — how likely a point of EXP is to buy Health rather than Attack, 50–100% (0% for Shedinja).
...
- Evolution triggers automatically once a mon crosses its EXP threshold (reuse real Pokémon evolution chains via PokeAPI, restricted to your curated Gen 1–3 roster). **As built** (ADR 0009): every **12 points of lifetime EXP**, worth a flat **+3 Attack and +3 Health**. The species evolved into contributes nothing else — a mon grows off the tier line of the species it *started* as for its whole life, so a Charizard is a Charmander with EXP and two evolutions behind it. At about four points a Location that puts the first evolution in the third Location and the last in the sixth. EXP is lifetime and never caps, so a final form keeps growing. Every mon the run owns is kept within 2 points of its most-experienced. The three branching lines in the roster — Eevee, Tyrogue, Nincada — don't evolve at all yet: picking a branch needs a choice the player makes, which isn't built.
- **Combine 2 of the same mon** to instantly grant EXP to one of them (consumes the duplicate) — a sacrifice/fusion mechanic for dupes. **As built:** dragging one onto another of the same species on the Team screen, which asks whether that meant combine or reorder; the mon dropped onto survives and gains one point of EXP — the same as a win — and the duplicate's own EXP is not carried over. Since growth is drawn per mon (ADR 0009), the two duplicates have different stat lines by mid-run, so which one you combine *into* is a real choice; the dialog quotes the outcome.
...
**As built** (ADR 0007): a badge counts toward the eight that win the run and refills Morale; the relic-style passive bonus isn't built yet.

---

## 15. Morale, Win & Loss

- Morale starts at some fixed value and decrements on a lost battle node. **As built (ADR 0007):** 3, and each badge restores it to full, so it budgets losses per Location.
- `Morale <= 0` → run over, "Better luck next time" screen, achievements recorded regardless.
- The run's win condition, **as built (ADR 0007):** eight badges (one per Location's Gym).
...
- **What should Event and PvP nodes pay?** PvP is still a stub. Events now pay money, items or a trade, but no EXP unless the Legendary is fought and beaten (ADR 0014), so a path through them still earns less. The pacing (ADR 0008) assumes a typical path of about 2.5 wild wins plus a Gym per Location — four points. The encounters' numbers ($5/$20 slots, a $15 bounty, 50% odds) are untuned.
- **How strong should an Event's Legendary be early on?** It's pitched at the Location's baseline EXP, so at the first badge a tier 4–6 Legendary will usually beat a starting pair (ADR 0014).
- **Fights should lengthen across a run, but by how much is untested** (ADR 0009). A point of EXP buys Attack *or* Health by the species' own growth value, so a bulky mon pulls away from its own Attack; a 50% grower stays close to its mirror. This replaced ADR 0008's permanent one-Step-per-mon exchange.
- **Speed 2 and Speed 3 are currently unreachable in play** (ADR 0009). An evolution ignores the new species' stats, including Speed, and encounters are drawn from base forms — so only the 20 already-fast base forms field anything above Speed 1. The Quick Claw (+1 Speed, ADR 0014) is now one other source; a reward or passive that grants Speed isn't built.
